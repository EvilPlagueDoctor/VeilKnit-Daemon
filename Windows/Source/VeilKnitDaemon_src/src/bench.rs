//! Automated crash-test course for Lifecycle.
//!
//! Each JSON file in `scenarios/` describes a pretend shutdown/startup situation: hooks that
//! succeed, fail, panic, run too long, block a worker, or wait on startup gates. This runner
//! builds those fake conditions and checks that Lifecycle reacts the way we expect.
//!
//! The point is to make shutdown bugs reproducible. Instead of staring at a long log and asking
//! "did that look right?", each scenario has explicit pass/fail expectations.
...
use crate::{
    ...
    lifecycle::{
        Budgets, HookResult, HookSpec, Lifecycle, Reachability, ResourceNeed, ShutdownPhase,
        StageOutcome, StopReason, WatchdogAction,
    },
};
...
enum Behaviour {
    ...
    Error,
    /// Deliberately violates the lifecycle hook contract by synchronously blocking the Tokio
    /// worker that polls the hook. This exercises the tier-level/uncooperative fallback.
    BlockThread,
    /// Occupies a runtime WORKER thread, not the blocking pool.
    ///
    /// Spawns an additional task that synchronously occupies a runtime worker. Used to stress
    /// the independent watchdog/canary path without relying on lifecycle itself to create a
    /// blocking boundary.
    StarveWorker,
...
async fn run_scenario(scenario: Scenario) -> Verdict {
    ...
    for hook in &scenario.hooks {
        ...
        let mut spec = HookSpec::new(def.module.clone(), parse_need(&def.need))
            .phase(parse_phase(&def.phase))
            .detail(format!("{:?} {}ms", def.behaviour, def.ms));
        if def.skip_on_restart {
            spec = spec.skip_on_restart();
        }
    ...
    // Report-mode watchdogs intentionally do not kill the bench process. If a scenario is
    // specifically asserting that the independent watchdog remains armed after an
    // uncooperative tier, give that OS thread enough time to report before checking it.
    if scenario.expect.watchdog_fired == Some(true) && !lifecycle.watchdog_fired() {
        tokio::time::sleep(budgets.watchdog.saturating_add(Duration::from_millis(25))).await;
    }
...
fn parse_phase(value: &str) -> ShutdownPhase {
    match value.to_ascii_lowercase().as_str() {
        "teardown" => ShutdownPhase::Teardown,
        "after" | "after_teardown" | "after-teardown" => ShutdownPhase::After,
        _ => ShutdownPhase::Automatic,
    }
}
