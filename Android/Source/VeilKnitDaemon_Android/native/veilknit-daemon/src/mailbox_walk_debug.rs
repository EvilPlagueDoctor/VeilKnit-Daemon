//! Mailbox walk timing diagnostics
//!
...
const TRACE_PATH: &str = "./user_data/mailbox-walk-debug.log";
...
pub fn log(message: String) {
    if diagnostics_to_stderr() {
        eprintln!("[mailbox-walk-debug] {message}");
    }

    SESSION_HEADER.call_once(|| {
        let _ = append_line(&format!(
            "========== mailbox walk diagnostic session: pid={} ==========",
            std::process::id()
        ));
    });

    if let Err(error) = append_line(&format!("[mailbox-walk-debug] {message}")) {
        if !FILE_WARNING_PRINTED.swap(true, Ordering::Relaxed) {
            eprintln!(
                "[mailbox-walk-debug] WARNING: could not append {}: {}",
                TRACE_PATH, error
            );
        }
    }
...
#[macro_export]
macro_rules! mailbox_walk_debug {
    ($($arg:tt)*) => {{
        $crate::mailbox_walk_debug::log(format!($($arg)*));
    }};
}
