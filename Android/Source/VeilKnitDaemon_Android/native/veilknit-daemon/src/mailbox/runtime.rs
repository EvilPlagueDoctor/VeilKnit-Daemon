        ...
        tokio::select! {
            ...
            command = rx.recv() => {
                ...
                match command {
                    ...
                    MailboxCommand::GetObservationReport { message_id, reply } => {
                        let report = runtime.persistent.observation_reports.get(&message_id).cloned();
                        let _ = reply.send(Ok(report));
                    }
                    MailboxCommand::ListObservationReports { reply } => {
                        let mut reports: Vec<_> = runtime
                            .persistent
                            .observation_reports
                            .values()
                            .cloned()
                            .collect();
                        reports.sort_by(|left, right| right.posted_at.cmp(&left.posted_at));
