...
enum MailboxCommand {
    ...
    GetObservationReport {
        message_id: [u8; 32],
        reply: oneshot::Sender<Result<Option<OutgoingMessageObservationReport>, MailboxError>>,
    },
    ListObservationReports {
        reply: oneshot::Sender<Result<Vec<OutgoingMessageObservationReport>, MailboxError>>,
    },
    ListInbox {
...
impl MailboxManager {
    ...
    pub async fn observation_report(
        ...
        let (reply_tx, reply_rx) = oneshot::channel();
        self.tx
            .send(MailboxCommand::GetObservationReport {
                message_id,
                reply: reply_tx,
            })
            .await
    ...
    pub async fn observation_reports(
        ...
        let (reply_tx, reply_rx) = oneshot::channel();
        self.tx
            .send(MailboxCommand::ListObservationReports { reply: reply_tx })
            .await
            .map_err(|_| MailboxError::ChannelClosed)?;
        reply_rx.await.map_err(|_| MailboxError::ChannelClosed)?
    }
