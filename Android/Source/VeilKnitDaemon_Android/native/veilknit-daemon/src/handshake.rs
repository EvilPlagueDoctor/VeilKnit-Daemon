...
fn validate_handshake_message(
    ...
    if !matches!(msg.message_type, 1 | 2 | 3 | 5) {
...
impl HandshakeManager {
            ...
            async move {
                ...
                if let Ok(message) = decode_and_validate_handshake_message(&data) {
                    ...
                    if let Err(err) = handshake
                        .process_validated_message(sender_dht, message)
                        .await
                    {
    ...
    async fn initiate_handshake_internal(
        ...
        if let Err(error) = send_handshake_message(&self.veilid, &blob.blob, &msg).await {
            self.emit_handshake_failed(
                &target_dht,
    ...
    pub async fn process_message(
        ...
        self.process_validated_message(sender_dht, msg).await
    }
    ...
    async fn process_validated_message(
        ...
        match msg.message_type {
            1 => {
                if let Some(existing) = self.sessions.get(&msg.sender_dht) {
                    if existing.status == HandshakeStatus::Established {
                        // A live application may need to upgrade a short walk/
                        // presence handshake into a maintained session. Accept
                        // that explicit upgrade instead of making the initiator
                        // wait for the three-minute quick session to expire.
                        if msg.maintain_connection && !existing.maintain_connection {
                            crate::tprintln!(
                            ...
                            crate::tprintln!(
                                "[handshake] Ignoring new init from already-established peer {}",
                                msg.sender_dht
                            );
                            return Ok(None);
                        }
            ...
            _ => {
                if !self.sessions.contains_key(&sender_dht) {
                    crate::tprintln!("[handshake] Unknown message type from unknown sender, ignoring");
                    return Ok(None);
                }
            }
        }
	...
	crate::tprintln!("[handshake] Sending type 2 to {:?}", blob);
        ...
        send_handshake_message(&self.veilid, &blob.blob, &reply).await?;
    ...
    async fn handle_welcome_reply(
        ...
        if let Err(error) = send_handshake_message(&self.veilid, route, &reply).await {
            state.status = HandshakeStatus::Failed;
            return Err(error);
        }
    ...
    pub async fn tick(&mut self) {
        ...
        for (peer_dht, state) in self.sessions.iter_mut() {
            ...
            if state.retries < MAX_HANDSHAKE_RETRIES && elapsed >= retry_delay {
			    ...
			    crate::tprintln!("we think a challenge has been recieved");
                            let shared = match state.session_key {
                                Some(k) => k,
                                None => {
                                    crate::tprintln!("[handshake] Missing key for retry with {}", state.peer_dht);
                                    failures.push((
                                        peer_dht.clone(),
                                        "Missing session key during handshake retry".to_string(),
                                        now.saturating_sub(state.started_at).saturating_mul(1_000),
                                        true,
                                    ));
                                    state.status = HandshakeStatus::Failed;
                            ...
                            let Some(their_challenge) = state.their_challenge.as_ref() else {
                                crate::teprintln!(
                                    "[handshake] Missing peer challenge for retry with {}",
                                    state.peer_dht
                                );
                                failures.push((
                                    peer_dht.clone(),
                                    "Missing peer challenge during handshake retry".to_string(),
                                    now.saturating_sub(state.started_at).saturating_mul(1_000),
                                    true,
                                ));
                                state.status = HandshakeStatus::Failed;
                    ...
                    if let Err(error) =
                        send_handshake_message(&self.veilid, route, &msg).await
                    {
                        crate::teprintln!(
                            "[handshake] Retry send failed for {}: {error}",
                            state.peer_dht
                        );
                    }
                } else {
