...
inline const char* ui_text(UiLanguage language, const char* english) {
    static const TranslationRow rows[] = {
        ...
        {"Restore backup", "Restaurer la sauvegarde", "Restaurar copia", "Восстановить копию", "恢复备份"},
        {"Enter the backup passphrase in the Password field, then choose the backup file.", "Saisissez la phrase secrète dans le champ Mot de passe, puis choisissez le fichier.", "Introduzca la frase de la copia en Contraseña y elija el archivo.", "Введите пароль резервной копии в поле пароля, затем выберите файл.", "在密码字段中输入备份口令，然后选择备份文件。"},
        {"Restoring encrypted backup…", "Restauration de la sauvegarde chiffrée…", "Restaurando copia cifrada…", "Восстановление зашифрованной копии…", "正在恢复加密备份…"},
        {"Backup restored. Log in with the original account password.", "Sauvegarde restaurée. Connectez-vous avec le mot de passe d’origine.", "Copia restaurada. Inicie sesión con la contraseña original.", "Копия восстановлена. Войдите с исходным паролем.", "备份已恢复。请使用原始账户密码登录。"},
        {"Backup restore failed. Check the file and backup passphrase.", "Échec de la restauration. Vérifiez le fichier et la phrase secrète.", "Error al restaurar. Compruebe el archivo y la frase.", "Не удалось восстановить. Проверьте файл и пароль.", "恢复失败。请检查文件和备份口令。"},
        {"Advanced application management", "Gestion avancée des applications", "Administración avanzada de aplicaciones", "Расширенное управление приложениями", "高级应用管理"},
        ...
        {"Allow checked", "Autoriser la sélection", "Permitir marcadas", "Разрешить отмеченные", "允许已勾选"},
        {"Application", "Application", "Aplicación", "Приложение", "应用程序"},
        {"Archive", "Archive", "Archivo", "Архив", "归档"},
        {"Discovery cache", "Cache de découverte", "Caché de descubrimiento", "Кэш обнаружения", "发现缓存"},
        {"Newest authorization request per application is shown. Check the applications you want, then allow or refuse the checked rows.", "La demande d’autorisation la plus récente de chaque application est affichée. Cochez les applications voulues, puis autorisez ou refusez les lignes cochées.", "Se muestra la solicitud de autorización más reciente de cada aplicación. Marque las aplicaciones deseadas y permita o rechace las filas marcadas.", "Показывается новейший запрос авторизации для каждого приложения. Отметьте нужные приложения, затем разрешите или отклоните отмеченные строки.", "每个应用仅显示最新的授权请求。勾选所需应用，然后允许或拒绝已勾选的项目。"},
        {"Observed applications", "Applications observées", "Aplicaciones observadas", "Обнаруженные приложения", "已发现的应用"},
        {"Recent", "Récent", "Recientes", "Недавние", "近期"},
        {"Refresh requests", "Actualiser les demandes", "Actualizar solicitudes", "Обновить запросы", "刷新请求"},
        {"Refusal reason", "Motif du refus", "Motivo del rechazo", "Причина отказа", "拒绝原因"},
        {"Refuse checked", "Refuser la sélection", "Rechazar marcadas", "Отклонить отмеченные", "拒绝已勾选"},
        {"Request", "Demande", "Solicitud", "Запрос", "请求"},
        {"Verified headers", "En-têtes vérifiés", "Encabezados verificados", "Проверенные заголовки", "已验证标头"},
        ...
        {"Names shown to applications", "Noms affichés aux applications", "Nombres mostrados a las aplicaciones", "Имена, показываемые приложениям", "向应用程序显示的名称"},
        {"Visible name", "Nom visible", "Nombre visible", "Видимое имя", "可见名称"},
        {"Set default visible name", "Définir le nom visible par défaut", "Establecer nombre visible predeterminado", "Задать видимое имя по умолчанию", "设置默认可见名称"},
        {"Set selected app alias", "Définir l’alias de l’application sélectionnée", "Establecer alias de la aplicación seleccionada", "Задать псевдоним выбранного приложения", "设置所选应用别名"},
        {"Clear selected alias", "Effacer l’alias sélectionné", "Borrar alias seleccionado", "Удалить выбранный псевдоним", "清除所选别名"},
        {"List name settings", "Lister les paramètres de nom", "Listar ajustes de nombre", "Показать настройки имён", "列出名称设置"},
        {"Network profiles", "Profils réseau", "Perfiles de red", "Сетевые профили", "网络配置文件"},
        {"New profile name", "Nom du nouveau profil", "Nombre del perfil nuevo", "Имя нового профиля", "新配置文件名称"},
        {"Create profile", "Créer le profil", "Crear perfil", "Создать профиль", "创建配置文件"},
        {"List profiles", "Lister les profils", "Listar perfiles", "Показать профили", "列出配置文件"},
        {"Profile id", "Identifiant du profil", "Id. del perfil", "Идентификатор профиля", "配置文件 ID"},
        {"Use after restart", "Utiliser après redémarrage", "Usar tras reiniciar", "Использовать после перезапуска", "重启后使用"},
        {"Retire profile", "Retirer le profil", "Retirar perfil", "Вывести профиль из использования", "停用配置文件"},
        {"Profile changes take effect after a safe daemon restart.", "Les changements de profil prennent effet après un redémarrage sûr du démon.", "Los cambios de perfil se aplican tras reiniciar el demonio de forma segura.", "Изменения профиля вступают в силу после безопасного перезапуска демона.", "配置文件更改会在安全重启守护进程后生效。"},
