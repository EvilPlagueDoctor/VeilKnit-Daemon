...
impl AppStorageManager {
    ...
    pub async fn list_stores(&self, app: &AuthenticatedAppSession) -> Vec<AppStoreDescriptor> {
        ...
        catalog
            ...
            .map(|stores| stores.iter().map(|store| store.descriptor.clone()).collect())
            .unwrap_or_default()
    }
    ...
    pub async fn create_store(
        ...
        {
            let catalog = self.catalog.lock().await;
            if catalog.stores.get(&app_id).map_or(0, Vec::len) >= MAX_APP_STORES_PER_APP {
...
pub struct AppSigningIdentity {
    ...
    pub created_at: u64,
    /// This is a daemon-authenticated app key bound to the local account at
    /// issuance time. Applications should publish/pin it in their room or
    /// profile data before trusting it remotely.
    pub binding: String,
}
