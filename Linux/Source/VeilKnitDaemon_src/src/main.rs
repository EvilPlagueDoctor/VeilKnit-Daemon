...
use node::*;
use network_events::StartupStage;
...
async fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    // Subkey 10 was also only ever read, never published. Advertise the
    // modules that actually started and the enabled canonical app ids known to
    // IdentityManager. Numeric app ids remain available for legacy apps.
    let application_info_stage = supervisor
        .stage_running(StartupStage::ApplicationInfo, None)
        .await;
    if let Some(runtime) = &main_dht_runtime {
        let mut capability_flags = CAPABILITY_PRIVATE_ROUTES
            | CAPABILITY_HANDSHAKE
            | CAPABILITY_REPUTATION
            | CAPABILITY_APP_AUTH;
        if walk_task.is_some() {
            capability_flags |= CAPABILITY_NETWORK_WALK;
        }
        if mailbox_manager.is_some() {
            capability_flags |= CAPABILITY_MAILBOX;
            if mailbox_participates_as_custodian {
                capability_flags |= CAPABILITY_MAILBOX_CUSTODIAN;
            }
        }

        let application_ids = identity_manager
            .list_apps()
            .await
            .into_iter()
            .filter(|registration| registration.enabled)
            .map(|registration| registration.app_id.to_string())
            .collect();
        ...
        if let Err(error) = runtime.publish_app_info(app_info).await {
...
fn login_or_signup(auth: &UserAuth) -> UserSession {
    loop {
        let choice = read_line(ui_language::t("Login or Signup? (l/s): "));
        let username = read_line(ui_language::t("Username: "));
        let password = read_line(ui_language::t("Password: ")); // plaintext echo - see note below

        let result = match choice.trim() {
            "l" | "L" => auth.login(&username, &password),
            "s" | "S" => auth.signup(&username, &password),
            _ => {
                crate::tprintln!("{}", ui_language::t("Please enter l or s."));
                continue;
            }
        };
