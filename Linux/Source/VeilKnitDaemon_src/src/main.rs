...
mod walk_settings;
mod walk_task;
pub(crate) use presentation::ui_language;
...
async fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    if !gui_bridge_mode {
        ui_language::select_console_language();
    }
    ...
    loop {
        ...
        let choice = read_line(ui_language::t("Command: "));
            ...
            match &local_api {
                Some(api) => {
                    ...
                    if gui_bridge_mode {
                        ...
                        crate::tprintln!("{}", ui_language::t("No application authorization requests are pending."));
                    } else {
                        crate::tprintln!("{}", ui_language::t("Pending application authorization requests:"));
                        ...
                        crate::tprintln!("{}", ui_language::t("Approve with: app-approve <request-id>"));
                        crate::tprintln!("{}", ui_language::t("Reject with: app-reject <request-id> [reason]"));
                    }
                }
...
fn login_or_signup(auth: &UserAuth) -> UserSession {
    loop {
        let choice = read_line(ui_language::t("Login, Signup, or Restore local backup? (l/s/r): "));
        if matches!(choice.trim(), "r" | "R") {
            let path = read_line(ui_language::t("Path to .veilknit-backup file: "));
            let passphrase = read_line(ui_language::t("Backup passphrase: "));
            match auth.restore_local_backup(path.trim(), &passphrase) {
                Ok(metadata) => crate::tprintln!(
                    "{} '{}' {}",
                    ui_language::t("Restored account"),
                    metadata.username,
                    ui_language::t("Log in with its original account password.")
                ),
                Err(error) => crate::tprintln!("{}: {error}", ui_language::t("Could not restore backup")),
            }
            continue;
        }

        let username = read_line(ui_language::t("Username: "));
        let password = read_line(ui_language::t("Password: ")); // platform GUIs mask this field

        let result = match choice.trim() {
            "l" | "L" => auth.login(&username, &password),
            "s" | "S" => auth.signup(&username, &password),
            _ => {
                crate::tprintln!("{}", ui_language::t("Please enter l, s, or r."));
                continue;
            }
        };
        ...
        match result {
            ...
            Err(AuthError::UserNotFound) => crate::tprintln!("{}", ui_language::t("No account with that username.")),
            Err(AuthError::UserAlreadyExists) => crate::tprintln!("{}", ui_language::t("That username is already taken.")),
            Err(AuthError::WrongPassword) => crate::tprintln!("{}", ui_language::t("Wrong password.")),
            Err(AuthError::InvalidUsername) => {
                crate::tprintln!("{}", ui_language::t("Usernames may only contain letters, numbers, '_' and '-'."))
            }
...
fn print_command_help() {
    crate::tprintln!("{}", ui_language::t("Core commands:"));
    crate::tprintln!("  mail ...              {}", ui_language::t("Mailbox status, inbox, retrieval, and maintenance"));
    crate::tprintln!("  app-pending/approve/reject  {}", ui_language::t("Review first-run app authorization"));
    crate::tprintln!("  app-add/list/rotate          {}", ui_language::t("Manage authenticated local applications"));
    crate::tprintln!("  app-name ...                 {}", ui_language::t("Set the default or per-app visible name"));
    crate::tprintln!("  profile-list/create/use/retire {}", ui_language::t("Manage isolated network profiles"));
    crate::tprintln!("  backup-local <path>   {}", ui_language::t("Create an encrypted .veilknit-backup archive"));
    crate::tprintln!("  backup-inspect <path> {}", ui_language::t("Verify and describe a local backup"));
    crate::tprintln!("  recovery-upload <path> {}", ui_language::t("Upload an encrypted backup to a random recovery DHT"));
    crate::tprintln!("  recovery-download <code> <path> {}", ui_language::t("Download a recovery archive"));
    crate::tprintln!("  recovery-status/wipe  {}", ui_language::t("Inspect or wipe the configured recovery record"));
    crate::tprintln!("  log or U              {}", ui_language::t("Save the current session log"));
    crate::tprintln!("  T/P/O                 {}", ui_language::t("Start, inspect, or stop a network walk"));
    crate::tprintln!("  H/K                   {}", ui_language::t("Start or inspect a handshake"));
    crate::tprintln!("  I                     {}", ui_language::t("Show internal nodes"));
    crate::tprintln!("  D                     {}", ui_language::t("Show daemon and DHT status"));
    crate::tprintln!("  Q                     {}", ui_language::t("Save and shut down"));
    crate::tprintln!("{}", ui_language::t("Application linking:"));
    crate::tprintln!("  {}", ui_language::t("Open the app once, note its request number, then use app-pending and app-approve <request-id>."));
