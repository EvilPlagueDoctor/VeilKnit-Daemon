...
Run `Windows\Source\VeilKnitDaemon_src\build_release_private.ps1`, or `Windows\build_daemon_and_mailer.bat`. Do not publish `.pdb`, `.obj`, `.ilk`, `.map`, or `target` contents.

## Android

Create your own untracked `local.properties` or configure `ANDROID_HOME`, then run `Android/Source/VeilKnitDaemon_Android/build_android_release_private.bat` (or `.sh`). Public release signing keys must remain outside the repository.

## Linux

Run `Linux/build_all_linux.sh`. The scripts remap paths and strip the copied release executables.
